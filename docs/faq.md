...
Any peer that has a `selfEndpoint` defined in `peers.nix` is reachable at any time. Peers without a `selfEndpoint` are only reachable when they've initiated a connection themselves (WireGuard establishes a path dynamically). All addresses in `172.29.64.1`-`172.29.127.254` are routed over the VPN.
...
No. This is a split-tunnel VPN. Only packets addressed to `172.29.64.0/18` are encapsulated and routed through the VPN. All other traffic (web browsing, etc.) goes through your normal connection unchanged.
